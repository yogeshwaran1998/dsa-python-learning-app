...
- Underlying implementation uses a dynamic array (CPython `list`)
- On growth, CPython over-allocates by roughly **1/8 (~12.5%)** of the new size
  (`new_allocated = newsize + (newsize >> 3) + 6`), so `append` is **amortized O(1)**
